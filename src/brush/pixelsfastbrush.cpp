...
PixelsFastBrush::PixelsFastBrush(const ObjectModel& objectModel, TextureStorage& textureStorage)
    ...
    for (IdType faceId = 0; faceId < objectModel.getFacesNumber(); ++faceId) {
        ...
        int minX = fmax(0, BrushUtils::getMinUvX(face) * w);
        int maxX = fmin(BrushUtils::getMaxUvX(face) * w, w - 1);

        for (int x = minX; x <= maxX; ++x) {
            float xUv = x / (1.0 * w);
            int minY = fmax(0, BrushUtils::getMinY(face, xUv) * h);
            int maxY = fmin(h - 1, BrushUtils::getMaxY(face, xUv) * h);
