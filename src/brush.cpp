...
Brush::Brush(std::vector<Vertex> vertices, TextureStorage* textureStorage) : AbstractBrush(vertices, textureStorage) {
}

void Brush::paintTriangle(glm::vec2 *points, std::vector<std::pair<glm::i32vec2, std::pair<Color, Color>>> &diff) {
    int minX = round(fmin(points[0].x, fmin(points[1].x, points[2].x)) * textureStorage->getWidth());
    int maxX = round(fmax(points[0].x, fmax(points[1].x, points[2].x)) * textureStorage->getWidth());

    for (int x = fmax(0, minX); x <= fmin(textureStorage->getWidth(), maxX); x++) {
        int minY = round(Geometry::getMinY(points[0], points[1], points[2], x / (1.0 * textureStorage->getWidth()))
                         * textureStorage->getHeight());
        int maxY = round(Geometry::getMaxY(points[0], points[1], points[2], x / (1.0 * textureStorage->getWidth()))
                         * textureStorage->getHeight());

        for (int y = fmax(0, minY); y <= fmin(textureStorage->getHeight(), maxY); y++) {
...
std::vector<std::pair<glm::i32vec2, std::pair<Color, Color>>>
    ...
    for (int i = 0; i < POINTS_CNT; i++) {
        glm::vec3 ray1 = fromScreenCoordinates(glm::vec2(centerPoint.x + radius * cos(2 * M_PI * i / POINTS_CNT) / textureStorage->getWidth(),
                                                         centerPoint.y + radius * sin(2 * M_PI * i / POINTS_CNT) / textureStorage->getHeight()),
                                                 matrixProjection);
        glm::vec3 ray2 = fromScreenCoordinates(glm::vec2(centerPoint.x + radius * cos(2 * M_PI * (i + 1) / POINTS_CNT) / textureStorage->getWidth(),
                                                         centerPoint.y + radius * sin(2 * M_PI * (i + 1) / POINTS_CNT) / textureStorage->getHeight()),
                                                 matrixProjection);
