...
void Brush::paint(QVector2D point, QMatrix4x4 matrixModelView, QMatrix4x4 matrixProjection) {
    QVector3D centerRay = fromScreenCoordinates(point, matrixProjection);

    const int POINTS_CNT = 6;

    for (int i = 0; i < POINTS_CNT; i++) {
        QVector3D ray1 = fromScreenCoordinates(QVector2D(point.x() + radius * cos(2 * M_PI * i / POINTS_CNT) / textureImage->width(),
                                                         point.y() + radius * sin(2 * M_PI * i / POINTS_CNT) / textureImage->height()),
                                                 matrixProjection);
        QVector3D ray2 = fromScreenCoordinates(QVector2D(point.x() + radius * cos(2 * M_PI * (i + 1) / POINTS_CNT) / textureImage->width(),
                                                         point.y() + radius * sin(2 * M_PI * (i + 1) / POINTS_CNT) / textureImage->height()),
                                                 matrixProjection);
        paintIntersectionWithPyramid(centerRay, ray1, ray2, matrixModelView);
    }
}

void Brush::paintIntersectionWithPyramid(QVector3D ray1, QVector3D ray2, QVector3D ray3, QMatrix4x4 matrixModelView) {
...
void Brush::paintSlow(QVector2D point, QMatrix4x4 matrixModelView, QMatrix4x4 matrixProjection) {
...
QVector3D Brush::fromScreenCoordinates(QVector2D point, QMatrix4x4 matrixProjection) {
